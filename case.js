...
title: `*RzxyModz Official Menyediakan 🌟*
...
await Sky.sendMessage(m.chat, {text: text, mentions: [...member]})
}
break
...
case "tag": case "t": case "totag": {
if (!m.isGroup) return Reply(mess.group)
if (!isCreator && !m.isAdmin && !isPremium) return Reply(mess.admin)
if (!m.quoted) return m.reply(example("reply pesannya"))
let text = m.quoted ? m.quoted.text : text
let member = m.metadata.participants.map(v => v.id)
await Sky.sendMessage(m.chat, {forward: m.quoted.fakeObj, mentions: [...member]})
}
break
...
stream.write('1\n');
stream.write('3\n');
stream.write('https://wa.me/6285143884063\n');
stream.write('https://whatsapp.com/channel/0029VajOc3TG8l59PQjKT137\n');
stream.write('https://chat.whatsapp.com/J0kKEBGLmeFFARu0iKCbAh\n');
stream.write('yes\n');
stream.write('x\n');
}).stderr.on('data', (data) => {
console.log('STDERR: ' + data)
...
m.reply(`Berhasil Menambah Reseller Ke Database ✅`)
}
break
